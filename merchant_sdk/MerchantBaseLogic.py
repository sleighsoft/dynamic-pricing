...
base_settings = {}
conf = config.get_config()['merchant_config']
...
class MerchantBaseLogic:
    ...
    @staticmethod
    def calculate_id():
        token = os.getenv('PRICEWARS_MARKETPLACE_TOKEN', token)
        return base64.b64encode(hashlib.sha256(token.encode('utf-8')).digest()).decode('utf-8')

    @staticmethod
    def get_marketplace_url():
        marketplace_url = os.getenv('PRICEWARS_MARKETPLACE_URL', conf['marketplace_url'])
    ...
    def get_producer_url():
        producer_url = os.getenv('PRICEWARS_PRODUCER_URL', conf['producer_url'])
    ...
    def get_kafka_reverse_proxy_url():
        url = os.getenv('PRICEWARS_KAFKA_REVERSE_PROXY_URL', conf['kafka_reverse_proxy_url'])
